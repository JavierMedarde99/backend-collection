...
class BookPersistenceAdapterTest {
    ...
    @Test
    void findByIsbn_mapsEntity_whenExists() {
        BookEntity entity = BookEntity.builder().id("b1").isbn("9780307474728").build();
        Book expected = sampleBook();
        when(springDataBookRepository.findByIsbn("9780307474728")).thenReturn(Optional.of(entity));
        when(mapper.toDomain(entity)).thenReturn(expected);

        Optional<Book> result = adapter.findByIsbn("9780307474728");
